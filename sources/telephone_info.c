...
void Add(base_t *base, unsigned long networkCode, unsigned long number,
  ...
  if (nameOwner != NULL) {
    base->telephones_[base->telephoneCount_].nameOwner_ =
        malloc((strlen(nameOwner) + 1));
...
bool IsValidNumber(unsigned long networkCode, unsigned long number) {
  return networkCode <= MAX_NETWORK_CODE &&
...
void Sort(base_t *base) {
  ...
  for (unsigned long i = 1; i < base->telephoneCount_; ++i) {
    for (unsigned long j = 0; j < base->telephoneCount_ - 1; ++j) {
      if (base->telephones_[j].networkCode_ >
          base->telephones_[j + 1].networkCode_) {
